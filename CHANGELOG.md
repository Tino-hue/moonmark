...
九月黑客松维护周期：分析引擎真实化（F01–F06）、跨平台收尾（F07）、QuickCheck 属性测试接入、CI 三平台实证。

### Added

- **F01 真实 API 计数驱动废弃密度** (`600fa53`)：废弃密度评分由估算改为统计包内真实公共 API 数量，评分有据可依。
- **F02 传递性体积评分 + Top 5 Size Offenders** (`6f81705`)：体积维度接入传递性归因，终端报告输出可行动的体积排序列表。
- **F04 DEPRECATED-002 间接废弃诊断** (`8de1d70`)：反向 BFS 传播链完整可视化，"谁传染了谁"逐跳可追溯。
- **F05 `depsight stats` 命令** (`e632d6d`)：21 项生态全景指标，终端 + `--json` 双输出。
- **F06 全离线演示场景** (`3db7ebc`, `79202b6`)：`examples/risky_project` 7 个 mock 包离线触发全部 5 种诊断码（CYCLE-001 / LICENSE-001/002 / DEPRECATED-001/002），新增模拟过时依赖场景；评审/演示零网络依赖。
- **F07 跨平台路径统一** (`69c1d9a`, `103e1c4`)：全部路径拼接收敛到 `FileSystem.path_join` 抽象（Node.js `path.join` FFI），代码层 Windows/macOS/Linux 同码运行。
- **QuickCheck 属性测试** (`d1ec044`)：接入 v0.10.9 工具链的 `moonbitlang/core/quickcheck`，为 SemVer 模块编写 8 条属性 × 100 随机用例——`compare` 全序三公理（自反/反对称/传递）、parse/render round-trip、`^`/`~`/`=`/`>=` 四条约束 iff 语义规格。反例经 `counterexample_context` 渲染为可读版本字符串。测试专用依赖，不进入生产构建图。
- **CI 三平台矩阵** (`1628ab5`, `310b0a4`, `8e9e6e0`)：`build-and-test` 扩展为 `ubuntu-latest` / `windows-latest` / `macos-latest` 三变体（`fail-fast: false`），Unix/Windows 步骤按 `runner.os` 配对。Windows 走 pwsh 镜像链安装（官方 `powershell.ps1`），示例回归断言与 Unix 逐一等价。

### Changed

- **工具链基线**：`moonc v0.10.11 / moon 0.1.20260827`，已包含 v0.10.9 全部特性（QuickCheck、`errdefer`、guard 完备性检查等），零迁移成本。
- **CI 加固**：`curl|bash` 退出码掩码修复 + 硬校验 moon 二进制（`c3ee213`）；自审计 job 网络瞬断重试与 `&&` 模式消除（`42866f3`, `87b5cbf`）；JS FFI `Array` → `FixedArray` 消除 W0020 弃用警告（`fa8fa8c`）。
- 测试套件：**267 → 282 tests**（+15：QuickCheck 8 条属性 + 本周期前置收尾 7 条用例）。

### Verified (v0.10.11 toolchain, 2026-09-08)

- `moon test --target js`: **282 / 282 通过**, EXIT 0（Ubuntu / Windows / macOS 三平台 CI 实证）
- `moon check --target js`: 0 errors, 0 warnings
- `moon fmt --check`: EXIT 0
- `moon info`: EXIT 0
- `moon build --target js`: EXIT 0，JS bundle 单文件输出
